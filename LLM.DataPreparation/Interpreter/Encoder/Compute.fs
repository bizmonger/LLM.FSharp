...
module Compute =
    ...
    let softmaxSpan (input: ReadOnlySpan<float>) : float[] =
        ...
        if input.Length = 0 then [||]
            ...
            let expValues = Array.zeroCreate<float> input.Length 

            // 4. Calculate the exponentials of (x - maxVal) for each element x in the input span.
            for i = 0 to input.Length - 1 do
                expValues.[i] <- Math.Exp(input.[i] - maxVal)
            ...
            if sumExp = 0.0 then
                ...
                let result = Array.zeroCreate<float> input.Length
